...
while true; do
    ...
    if [ "$active_postgres" = "active" ]; then
        active_node_1=$(psql -h 127.0.0.1 -At  -d repmgr -U repmgr -c 'select active from nodes where node_id=1;')
        active_node_2=$(psql -h 127.0.0.1 -At  -d repmgr -U repmgr -c 'select active from nodes where node_id=2;')

        {% if ha_role == "main" %}

        database_role=$(psql -h 127.0.0.1 -At  -d repmgr -U repmgr -c 'select type from nodes where node_id=1;')
        ...
        database_role=$(psql -h 127.0.0.1 -At  -d repmgr -U repmgr -c 'select type from nodes where node_id=2;')
